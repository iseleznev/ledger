...
import static org.seleznyov.iyu.kfin.ledgerservice.core.constants.AccountPartitionMmemHashTableConstants.SNAPSHOT_INDEX_HEADER_SIZE;
import static org.seleznyov.iyu.kfin.ledgerservice.core.constants.AccountPartitionMmemHashTableConstants.SNAPSHOT_INDEX_MAGIC;
import static org.seleznyov.iyu.kfin.ledgerservice.core.utils.OrdinalUtils.nextOrdinal;

public class AccountPartitionLazyResizeMmemHashTable implements AccountPartitionHashTable {
    ...
    public AccountPartitionLazyResizeMmemHashTable(
        int partitionNumber,
        int chunkNumber,
        Arena arena,
        LedgerConfiguration ledgerConfiguration,
        long expectedCapacitySize,
        int maxAcceptedPsl
    ) {
        this.capacitySize = Math.max(nextPowerOfTwo(expectedCapacitySize * 2L), HASH_TABLE_PREALLOC_CHUNK);
        this.mask = (int) capacitySize - 1;
        this.arena = arena;
        this.directoryPath = Path.of(ledgerConfiguration.snapshot().path()).resolve(
            "account-partition-" + partitionNumber + "-" + chunkNumber + ".mmem"
        );

        this.size = 0;
        this.totalProbes = 0;
        this.maxPsl = 0;
        this.maxAcceptedPsl = (int) expectedCapacitySize;
        this.partitionId = partitionNumber;
    ...
    public void initialize() throws IOException {
        ...
        try (FileChannel channel = FileChannel.open(
            ...
            long indexSize = SNAPSHOT_INDEX_HEADER_SIZE + capacitySize;

            // Выделяем временный segment для header
            MemorySegment header = Arena.ofAuto().allocate(SNAPSHOT_INDEX_HEADER_SIZE);

            // Заполняем header
            header.set(ValueLayout.JAVA_LONG, 0, SNAPSHOT_INDEX_MAGIC);
            header.set(ValueLayout.JAVA_INT, 8, 1);  // version
            header.set(ValueLayout.JAVA_INT, 12, partitionId);
            header.set(ValueLayout.JAVA_LONG, 16, capacitySize);
            header.set(ValueLayout.JAVA_LONG, 24, 0L);  // count
            header.set(ValueLayout.JAVA_INT, 32, (int)(HASH_TABLE_LOAD_FACTOR * 100));
            header.set(ValueLayout.JAVA_LONG, 36, System.currentTimeMillis());
            header.set(ValueLayout.JAVA_LONG, 44, System.currentTimeMillis());
            ...
            System.out.println("Created index file: " + this.directoryPath +
                ", size: " + indexSize + " bytes");
        }

        this.memorySegment = fileChannel.map(
            FileChannel.MapMode.READ_WRITE,
            0,
            (long) HASH_TABLE_ENTRY_SIZE * capacitySize,
            this.arena
        );
        initializeMemory();
    }
    ...
    public void flushWithMetaData() throws IOException {
        if (this.fileChannel != null && this.fileChannel.isOpen()) {
    ...
    private void initializeMemory() {
        ...
        long totalBytes = (long) capacitySize * HASH_TABLE_ENTRY_SIZE;
    ...
    private void startLazyResize() {
        ...
        oldCapacity = capacitySize;
        oldMask = mask;

        // Выделяем новую память
        this.capacitySize = nextPowerOfTwo(newCapacity);
        this.mask = capacitySize - 1;
        this.memorySegment = arena.allocate((long) HASH_TABLE_ENTRY_SIZE * capacitySize);
        this.maxPsl = 0;
    ...
    private long putInternal(long msb, long lsb, long balance, long ordinal, long count,
        ...
        while (currentPsl <= maxAcceptedPsl) {
            ...
            if (state == OCCUPIED) {
                ...
                int existingPsl = (currentIndex - existingIdeal + capacitySize) & mask;
    ...
    public long put(UUID accountId, long balance, long ordinal, long count) {
        ...
        if (size >= capacitySize * HASH_TABLE_LOAD_FACTOR && !isRehashing) {
            startLazyResize();
        }
...
//    }
    ...
    private void resize(int newCapacity) {
        ...
        final int oldCapacity = capacitySize;
    ...
    public double loadFactor() {
        return (double) size / capacitySize;
    }
    ...
    public String getStats() {
        return String.format(
            ...
            size, capacitySize, loadFactor(), averageProbeLength(), maxPsl
        );
    }
