...
public class AccountsPartitionActorPartitionAmountProcessor {

    private final SWSRRingBufferHandler partitionAmountRingBufferHandler;

    public AccountsPartitionActorPartitionAmountProcessor(SWSRRingBufferHandler partitionAmountRingBufferHandler) {
        this.partitionAmountRingBufferHandler = partitionAmountRingBufferHandler;
    }

    public void processAmounts(AccountsPartitionHashTable accountsPartitionHashTable, LedgerConfiguration ledgerConfiguration, long actorPartitionId) {
        ...
        if (partitionAmountProcessedSize >= 0) {
            long partitionProcessOffset = partitionAmountRingBufferHandler.readOffset();
            for (long offset = partitionAmountRingBufferHandler.readOffset(); offset < partitionProcessOffset + partitionAmountProcessedSize; offset += POSTGRES_ENTRY_RECORD_SIZE) {
