...
public class SharedAccountStateHashTable {
    ...
    private static final int REPLACING = 2;
    ...
    private static final int VALUE_ACTIVE_AREA_SIZE = COUNT_OFFSET + LONG_TYPE_SIZE;
    private static final int STATE_OFFSET = COUNT_OFFSET + LONG_TYPE_SIZE;
    private static final int PSL_OFFSET = STATE_OFFSET + INT_TYPE_SIZE;
    private static final int SHADOW_MSB_OFFSET = CPU_CACHE_LINE_SIZE;
    private static final int SHADOW_LSB_OFFSET = SHADOW_MSB_OFFSET + LONG_TYPE_SIZE;
    private static final int SHADOW_BALANCE_OFFSET = SHADOW_LSB_OFFSET + LONG_TYPE_SIZE;
    private static final int SHADOW_ORDINAL_OFFSET = SHADOW_BALANCE_OFFSET + LONG_TYPE_SIZE;
    private static final int SHADOW_COUNT_OFFSET = SHADOW_ORDINAL_OFFSET + LONG_TYPE_SIZE;
    private static final int SHADOW_COUNT_BOUND_OFFSET = SHADOW_COUNT_OFFSET + LONG_TYPE_SIZE;

    private static final int PSL_SIZE = 4;

    public static final int MAP_ENTRY_SIZE = (SHADOW_COUNT_BOUND_OFFSET + CPU_CACHE_LINE_SIZE - 1) & -CPU_CACHE_LINE_SIZE;
...
//    private static final int MAX_PSL = 16; // Максимальный Probe Sequence Length
    ...
    public SharedAccountStateHashTable(Arena arena, int expectedCapacity, int maxAcceptedPsl) {
...
//            throw new IllegalStateException("Hash table reached maximum capacity");
//        }

//        if (loadFactor() >= 1.0) {
//            throw new IllegalStateException("Invalid load factor: " + LOAD_FACTOR);
//        }
            ...
            while (currentPsl <= maxAcceptedPsl) {
                ...
                if (state == OCCUPIED) {
                    ...
                    if (currentPsl > existingPsl) {
                        ...
                        long tempBalance = memorySegment.get(JAVA_LONG, currentOffset + SHADOW_BALANCE_OFFSET);
                        long tempOrdinal = memorySegment.get(JAVA_LONG, currentOffset + SHADOW_ORDINAL_OFFSET);
                        long tempCount = memorySegment.get(JAVA_LONG, currentOffset + SHADOW_COUNT_OFFSET);
...
//        return put(key, balance, ordinal, count);
    ...
    public long getOffset(UUID key) {
        ...
        while (psl <= maxSearch) {
            ...
            if (state == OCCUPIED || state == REPLACING) {
                ...
                if (safetyOrderedState == REPLACING) {
                    long entryMsb = memorySegment.get(JAVA_LONG, offset + SHADOW_MSB_OFFSET);
                    long entryLsb = memorySegment.get(JAVA_LONG, offset + SHADOW_LSB_OFFSET);
                    if (entryMsb == msb && entryLsb == lsb) {
                        return offset;
                    }
                }
        ...
        while (psl <= maxSearch) {
            ...
            if (state == OCCUPIED || state == REPLACING) {
                ...
                if (safetyOrderedState == REPLACING) {
                    ...
                    long entryLsb = memorySegment.get(JAVA_LONG, offset + SHADOW_LSB_OFFSET);
                    if (entryMsb == msb && entryLsb == lsb) {
                        return memorySegment.get(JAVA_LONG, offset + SHADOW_BALANCE_OFFSET);
                    }
                }
            }
        ...
        while (psl <= maxSearch) {
            ...
            if (state == OCCUPIED || state == REPLACING) {
                ...
                if (safetyOrderedState == REPLACING) {
                    ...
                    long entryLsb = memorySegment.get(JAVA_LONG, offset + SHADOW_LSB_OFFSET);
                    if (entryMsb == msb && entryLsb == lsb) {
                        return memorySegment.get(JAVA_LONG, offset + SHADOW_ORDINAL_OFFSET);
                    }
                }
            }
    ...
    public long nextOrdinal(long offset) {
        final long nextOrdinal = memorySegment.get(JAVA_LONG, offset + ORDINAL_OFFSET) + 1;
        ...
        while (psl <= maxSearch) {
            ...
            if (state == OCCUPIED || state == REPLACING) {
                ...
                if (safetyOrderedState == REPLACING) {
                    ...
                    long entryLsb = memorySegment.get(JAVA_LONG, offset + SHADOW_LSB_OFFSET);
                    if (entryMsb == msb && entryLsb == lsb) {
                        return memorySegment.get(JAVA_LONG, offset + SHADOW_COUNT_OFFSET);
                    }
                }
            }
