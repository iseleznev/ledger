 ...
 */
@Slf4j
public class CommonRecordDirectPostgresBatchSender {

    private final DataSource dataSource;

    // ✅ Оптимальные настройки для network transmission
    private static final int OPTIMAL_CHUNK_SIZE = 256 * 1024; // 256KB
    private static final int CONNECTION_TIMEOUT_MS = 30_000;
    private static final int QUERY_TIMEOUT_MS = 60_000;
    private static final int RESULTS_ARRAY_LENGTH = 2;
    private static final int TOTAL_SIZE_RESULTS_ARRAY_INDEX = 0;
    private static final int ENTRIES_COUNT_RESULTS_ARRAY_INDEX = 1;
    ...
    private final AtomicLong totalEntriesSent = new AtomicLong(0);
    ...
    private final long[] sentResults = new long[RESULTS_ARRAY_LENGTH];

    public CommonRecordDirectPostgresBatchSender(DataSource dataSource) {
...
//        log.info("Created DirectQueryExecutorSender with chunk_size={}KB", OPTIMAL_CHUNK_SIZE / 1024);
    ...
    public long sendDirectly(BatchRingBufferHandler ringBufferLayout, long batchSlotOffset, long batchRawSize) {
        ...
        try (Connection connection = dataSource.getConnection()) {
            ...
            copyIteratively(copyIn, ringBufferLayout, batchSlotOffset, batchRawSize, sentResults);
            copyIn.flushCopy();
...
//            long remainingSize = totalSize - sent;
            long currentBatchSize = ringBufferLayout.batchSize(batchSlotOffset + sent);
            entriesCount += ringBufferLayout.batchElementsCount(batchSlotOffset + sent);
    ...
    public int sendViaOptimizedInputStream(MemorySegment postgresBinarySegment) throws SQLException {
        ...
        try (Connection connection = createOptimizedConnection()) {
            ...
            try (InputStream optimizedStream = new OptimizedPostgresBinaryInputStream(postgresBinarySegment)) {
                ...
                long insertedRows = copyManager.copyIn(COPY_SQL, optimizedStream);
